...
import { NO_PlAYER, HUMAN, CPU, TickTackToeBox, useTickTackToeStore } from "./TickTackToe.state";

const game = useTickTackToeStore();

function getBoxClass(box: TickTackToeBox) {
  const isWinningBox = game.winner !== undefined && game.winner.boxIndices.indexOf(box.index) >= 0;
  ...
  <div class="tick-tack-toe">
    ...
    <template v-if="game.playing">
      ...
      <div class="actions">
        ...
        <Button v-show="game.isDone" variant="primary" @click="game.restart()">Restart</Button>
      </div>
    </template>
  </div>
</template>
